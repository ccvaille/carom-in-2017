...
function deleteOldJSFile(newMd5Val) {
    var dir = path.resolve(__dirname, '../../../public/cdn/oms/js/');
...
function deleteOldCSSFile(newMd5Val) {
    var dir = path.resolve(__dirname, '../../../public/cdn/oms/css/');
    var arr = fs.readdirSync(dir);
    console.log(arr);
    for (var i = 0; i < arr.length; i++) {
        var item = arr[i];
...
var begin = function () {
    ...
    /*
     ...
     data =ClearBr(data);
     var data1 = fs.readFileSync(temp + '/src/dist/public/views/oms/index.html',"utf-8");
     data1 = data1.replace(/[\r\n]/g, "");
     data1 = data1.replace(/<script>(.*)<\/script>/g,"<script>" +data+ "</script>");
     fs.writeFile(temp + '/resources/views/index.blade.php',data1);
     */
//获取到生成的MD5串；
    var data1 = fs.readFileSync(temp + '/src/dist/public/oms/index.html', "utf-8");
...
//写入到新的文件中；
    fs.writeFile(temp + '/resources/views/oms/index.blade.php', data);

//复制公共部分的图片
    exists(temp + '/src/comm', temp + '/public/cdn/comm', copy);

//复制业务逻辑代码
    exists(temp + '/src/dist/public/oms', temp + '/public/cdn/oms', copy);
//删除生成的临时文件
    setTimeout(function () {
...
fs.access(path.resolve(__dirname, './../../dist/public/oms/index.html'), fs.R_OK | fs.W_OK, (err) => {
    if (err) {
        console.log(err);
        process.exit();
    } else {
