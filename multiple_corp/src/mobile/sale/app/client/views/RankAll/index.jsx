...
import * as rankAllActions from '../../actions/rankAllActions'
...
class RankAll extends React.Component {
    ...
    componentDidMount() {
        ...
        let { rankAllActions, rankAllReducers } = this.props;
        rankAllActions.getAuth();
        rankAllActions.fetchUserRankData({
            year: rankAllReducers.year,
            m: rankAllReducers.month,
            show: 1
        });
    }
    ...
    handleFilterChange = (value) => {
        let { rankAllActions, rankAllReducers } = this.props;
        let { tabIndex , month, year,dept} = rankAllReducers;
        rankAllActions.filterChange({
            year: value.year,
            month: value.month,
            dept: value.dept,
        });

        if (value.dept.id !== dept.id) {
            browserHistory.push({
                pathname: '/mobile/sale/app/rank/',
                state: value
            });
        }
        else {
            ...
            if (typeof (value.dept.type) === 'undefined' || value.dept.type === 1) {
                if (tabIndex === 0) {
                    ...
                    rankAllActions.fetchDeptRankData({
                        year: value.year,
                        m: value.month,
                        users: users,
                    });
    ...
    toggleView = (value) => {
        const { rankAllActions, rankAllReducers } = this.props;
        let { dept, tabIndex, month, year, ...others } = rankAllReducers;
        rankAllActions.switchTab(value);
    ...
    handleScroll = () => {
        const { rankAllActions, rankAllReducers } = this.props;
        if (rankAllReducers.isMore) {
            let vh = Math.max(document.documentElement.clientHeight, window.innerHeight || 0);
            let bottom_dis = ReactDOM.findDOMNode(this.refs.loadMore).getBoundingClientRect().bottom;
            if ((bottom_dis - 20) <= vh) {
                rankAllActions.pageChange(rankAllReducers.pageIndex + 1);
            }
        }
    }

    render = () => {
        let { rankData, pageData, dept, isMore, tabIndex, month, year, auth, fetching,...others } = this.props.rankAllReducers;

        return (
            <div className="sale-rank-all">
                <DepDateFilterPanel
                    dept={dept}
                    year={year}
                    month={month}
                            ...
                            pageData && pageData.length > 0 ? (
                                <div className="content">
                                    <table>
                                        <thead>
                                            <tr>
                                                <th className="name-title-cell" colSpan="2">排名</th>
                                                <th>金额</th>
                                                <th>单量</th>
                                            </tr>
                                        </thead>
...
const mapStateToProps = ({ rankAllReducers }) => ({
    rankAllReducers,
});

const mapDispatchToProps = dispatch => ({
    rankAllActions: bindActionCreators(rankAllActions, dispatch)
});

export default connect(mapStateToProps, mapDispatchToProps)(RankAll);
