...
import * as goalAllActions from '../../actions/goalAllActions'
...
class Goal extends React.Component {
    ...
    componentDidMount() {
        ...
        let { goalAllActions, goalAllReducers } = this.props;
        goalAllActions.getAuth();
    }
    componentWillUnmount() {
        document.removeEventListener('scroll',this.handleScroll,false); 
        let { goalAllActions, goalAllReducers } = this.props;
        goalAllActions.initState();
    ...
    handleFilterChange = (value) => {
        let { goalAllActions, goalAllReducers } = this.props;
        let { month, year, dept } = goalAllReducers;

        goalAllActions.filterChange({
            year: value.year,
            month: value.month,
            dept: value.dept,
        });
        if ((typeof value.dept.id !== 'undefined') && value.dept.id !== dept.id) {
            ...
            //如果是个人权限
            if (goalAllReducers.auth.target === 1) {
                goalAllActions.fetchGoalStat({
                    year: value.year,
                    m: value.month,
                    users: [],
                    show: 1,
            ...
            //如果是销售经理权限
            else if (goalAllReducers.auth.target === 2) {
                goalAllActions.fetchGoalStat({
                    year: value.year,
                    m: value.month,
                    users: users,
    ...
    renderWarnText = (data) => {
        let { month, year, ...others } = this.props.goalAllReducers;
    ...
    //下拉加载
     handleScroll=()=>{
        let { goalAllActions, goalAllReducers } = this.props;
        if (goalAllReducers.isMore) {
            let vh = Math.max(document.documentElement.clientHeight, window.innerHeight || 0);
            let bottom_dis = ReactDOM.findDOMNode(this.refs.loadMore).getBoundingClientRect().bottom;
            if ((bottom_dis - 20) <= vh) {
                goalAllActions.pageChange(goalAllReducers.pageIndex + 1);
            }
        }
    }

    handleRowClick = (item) => {
        let { month, year } = this.props.goalAllReducers;
        let dept = {
            name: item.name,
            id: item.key,
    ...
    render = () => {
        let { goalData, dept, auth, month, year, fetching, pageData, isMore, ...others } = this.props.goalAllReducers;
...
const mapStateToProps = ({ goalAllReducers }) => ({
    goalAllReducers,
});

const mapDispatchToProps = dispatch => ({
    goalAllActions: bindActionCreators(goalAllActions, dispatch)
});
