...
function goalReducers(state = initialState, action) {
	switch(action.type) {
		...
		case goalTypes.FETCHING:
			return {
				...state,
				fetching:true
			}
		case goalTypes.FETCH_AUTH_SUCCESS:
			return {
				...state,
				auth: action.payload,
			}
		case goalTypes.FILTER_CHANGE:
			return {
				...state,
				...action.payload
			}
		case goalTypes.FETCH_GOAL_SUCCESS:
			var _pageData=[];
