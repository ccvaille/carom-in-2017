...
baseConfig.entry.admin = __DEV__
    ? [`webpack-dev-server/client?http://${config.host}:${config.adminSecurity.port}/`, 'webpack/hot/only-dev-server', 'babel-polyfill', APP_ENTRY]
...
if (__DEV__) {
  ...
  baseConfig.output.publicPath = 'https://www.staticec.com/admin/security/';
  baseConfig.plugins.push(
    new HtmlWebpackPlugin({
