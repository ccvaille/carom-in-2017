...
class ViewMessage extends React.Component {
    ...
    componentWillMount() {
        this.props.fetchBroadcastMessage(1);
    }

    componentDidUpdate() {
        //滚动到当前窗口，需要异步处理一下
        var needShowIndex = this.props.broadcastData.needShowIndex;
        if (needShowIndex) {
            setTimeout(function () {
                if (this.props.broadcastData.curpage == 1) {
    ...
    componentDidMount() {
        this.props.bindScrollGetMore(function () {
            var state = this.props.broadcastData;
            if (state.end == 1 || state.loading) {
                return;
            }
            this.props.fetchBroadcastMessage(state.curpage + 1)
        }.bind(this));
    }
    ...
    render() {
        ...
        var hasMore = <div className="ecTeam-box ecTeam-box-sno">
            <div className="more-btn">
                <a href="javascript:;" onClick={() => this.props.fetchBroadcastMessage(state.curpage + 1)}>查看更多消息</a>
            </div>
        </div>
        ...
        return <div>
...

            {
                state.list && state.list.map((item, index) => {
                    return (<div ref={"item_" + index}><EnterpriseRadio data={item} userid={state.userid} /></div>)
                })
            }
        </div>
...
ViewMessage.contextTypes = {
    // broadcastData: PropTypes.object.isRequired,
