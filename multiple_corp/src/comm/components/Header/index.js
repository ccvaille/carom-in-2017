...
import getAuthFetch from './fetch.js'
  ...
  constructor(props) {
    ...
    componentDidMount() {
        ...
        if(!!window.ActiveXObject || "ActiveXObject" in window) {
          ...
          event.initEvent('auth', true, true);
        } else {
          event = new Event('auth');
        }
        getAuthFetch('auth', 'get')
          .then(json => {
            if (json.code == 200) {
              window.authData = json;
              window.dispatchEvent(event);
              return getAuthFetch('info', 'get')
              .then(json => {
              ...
              // 未登录
              location.href = 'https://www.workec.com/login?from=' + currentHref;
            }
          })

      let url = window.location.href;
      let tabArr = ['role', 'field', 'tag', 'rule', 'qq', 'sale'];

      let curTab = '';
      tabArr.some( (d, i) => {
        var tebRegExp = new RegExp('/'+ d);
        let _match = url.match(tebRegExp);
    ...
    render() {
        ...
        return (
            ...
            <div className="new-label-left">
                ...
                {
                myNavAuth && myNavAuth.indexOf(301001) !== -1 ? <li  className={ this.state.curTab === 'role' ? "active" : ''}><a href="https://corp.workec.com/corp/role?f=-1">权限管理</a></li> : null
                }{
                myNavAuth &&  myNavAuth.indexOf(301002) !== -1 ? <li  className={ this.state.curTab === 'field' ? "active" : ''}><a href="https://corp.workec.com/crm/set/field">自定义字段</a></li> : null
                }{
                myNavAuth &&  myNavAuth.indexOf(301003) !== -1 ? <li  className={ this.state.curTab === 'tag' ? "active" : ''}><a href="https://biz.workec.com/web/crm/tag">客户标签</a></li> : null
                }{
                myNavAuth &&  myNavAuth.indexOf(301004) !== -1 ? 
                <li  className={ this.state.curTab === 'rule' ? "active" : ''}>
                    {
                        this.state.crmStageGrayRelease == 1 ?
                        <a href="https://corp.workec.com/crm/set/stage">客户库规则</a>
                        :
                        <a href="https://html.workec.com/biz/web/crm/rule/stage.html">客户库规则</a>
                    }
                </li>
                : 
                null
                }{
                myNavAuth &&  myNavAuth.indexOf(301005) !== -1 ? <li  className={ this.state.curTab === 'qq' ? "active" : ''}><a href="https://corp.workec.com/crm/qq/index">QQ授权管理</a></li> : null
                }{
                myNavAuth &&  myNavAuth.indexOf(301006) !== -1 ? <li  className={ this.state.curTab === 'sale' ? "active" : ''}><a href="https://html.workec.com/biz/web/crm/sale/index.html">销售金额配置</a></li> : null
                }
                </ul>
            </div>
