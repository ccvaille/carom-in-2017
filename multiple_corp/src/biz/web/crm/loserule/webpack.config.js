...
baseConfig.entry.bizLoseRule = __DEV__
    ? [`webpack-dev-server/client?http://${config.host}:${config.bizLoseRule.port}/`, 'webpack/hot/only-dev-server', 'babel-polyfill', APP_ENTRY]
...
if (__DEV__) {
    ...
    baseConfig.output.publicPath = 'https://1.staticec.com/biz/web/crm/loserule/';
    baseConfig.plugins.push(
        new HtmlWebpackPlugin({
