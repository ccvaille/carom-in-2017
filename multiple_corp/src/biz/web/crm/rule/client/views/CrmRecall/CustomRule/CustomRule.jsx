...
import imgToSee from 'images/5.png'

const CheckboxGroup = Checkbox.Group;
const RadioGroup = Radio.Group;

const text = '使用“客户收回策略”能够促进客户资源的流转，帮助企业更大的发挥客户资源价值。';
const content = (
...
class CustomRule extends React.Component {
    ...
    componentDidMount() {
        ...
        this.props.ruleActions.getTipState();
    }
    //日期控件禁用的日期
    disableDate = (current) => {
        if (current) {
    ...
    closeTipModal = () => {
        this.props.ruleActions.setTipState(22);
    }
    ...
    render() {
        ...
        if(data.stage) {
            helpIntro = [
                ...
                '所有条件只针对处于客户阶段“新入库”~“'+ data.stage.names[2] +'”的客户，“'+ data.stage.names[3] +'”的客户不会受到影响；',
                        ...
                        <div>
                                    ...
                                    天，从未发生有效联系时;
                            {
                                        (this.isNumberAviliable(3, 365, data.conditionText1) || !data.isCheckCondition1) ? null :
                                            <span className="back_rule_error">请输入3~365以内的整数！</span>
                                    }
                                </p>
                                <p>
                                    ...
                                    天，从未发生有效联系时;
                            {
                                        (this.isNumberAviliable(7, 365, data.conditionText2) || !data.isCheckCondition2) ? null :
                                            <span className="back_rule_error">请输入7~365以内的整数！</span>
                                    }
                                </p>
                                <p>
                                    ...
                                    <p className="tips" style={{ lineHeight: '15px' }}>以下属于有效的联系方式
                            {
                                            (data.checkedContactWays.length > 0) ? null :
                                                <span className="back_rule_error">请勾选属于有效联系了客户的联系方式！</span>
                                        }
                                    </p>
                                    <div className={(data.checkedContactWays.length > 0) ? 'back-rule-check' : 'back-rule-check way-no-check'} >
                                        <CheckboxGroup
                                    ...
                                    该客户具备以下任意一个标签：
                            {
                                        (data.checkedSigns.length > 0 || !data.isCheckCustomSign) ? null :
                                            <span className="back_rule_error">请添加需要的标签！</span>
                                    }
                                </p>
                            ...
                            <DatePicker disabled={!(data.isCheckCondition1 || data.isCheckCondition2 || data.isCheckCondition3)}
                                        placeholder="未来30天以内日期" style={{ width: 135, marginRight: 10 }} disabledDate={this.disableDate}
                                ...
                                <p>
                                ...
                                </Button>
                                    {
                                        (data.isAlreadyEdit && isDataChanged) ?
                                            <span className="back_rule_error">同一天只可以修改一次！</span> : null
                                    }
                                </p>
                            </div>
                            ...
                            <Modal title="修改策略"
                                ...
                                <p style={{ padding: '10px 0' }}>收回策略每天只能修改一次，确定修改吗？</p>
                            </Modal>
                            <Modal title="关闭策略"
                                wrapClassName="vertical-center-modal"
                                width={390}
                                ...
                                <p style={{ padding: '10px 0' }}>确定关闭客户收回策略吗？</p>
                            </Modal>
                        </div>
                        : <CustomBackInfo onOpen={this.openBackRule} stage={data.stage} tipShow={ data.tipShow } closeTipModal={ this.closeTipModal }/>
                }
            </div>
        </div>);
    }
