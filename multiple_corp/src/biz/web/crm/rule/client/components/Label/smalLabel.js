...
import { Popover, Input, Modal, Button, message} from 'antd'
...
class SmallLabel extends React.Component {
	...
	setGroupCtrlIndex() {
		this.setState({
			...
			ctrlFromLastGroup: this.props.index === this.props.groups.length - 1
		});
	}
	renderGroupCtrl() {
		return (
	...
	groupClick() {
  	...
  	render () {
		...
		this.groupOperate = this.renderGroupCtrl();
    	return (
			<li id={"test" + this.props.labelId} className={classNames({'pop-active': !this.props.isSelect && this.state.visible}, this.props.isSelect ? 'active ' + fStyle : fStyle)}>
			{
				this.props.dataType =="group" ?
					...
					<a href="javascript:;" onClick={this.props.labelClick} className={this.props.isActive ? 'tag-item active' : 'tag-item' }>
						<span>{ this.props.labelName }</span>
					 	<Popover getPopupContainer={() => document.getElementById("test" + this.props.labelId) } 
