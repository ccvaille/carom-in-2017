...
import {Row, Col, Icon, Menu, Dropdown, Button, Checkbox, Radio, Select, Popover, Modal, Input, message} from 'antd';
import Message from '../Message';
import Crmcarousel from '../carousel';
...
import {
    ...
    setSelectLabelName, delLabel, selectGroupAct, setInputValue, dragSortLabel, dragSortGroup,
    multiDelLabel, multiSetGroup, getGroupList
...
class LeftGroup extends React.Component {
     sortDecorator = (componentBackingInstance) => {
    ...
    drop = (evt) => {
        ...
        const groups = this.props.postsByReddit.groupList.groups;
        const { isManageModal } = this.props.postsByReddit;

        let el = evt.item;
        let sibling = evt.to.childNodes[evt.newIndex + 1];

        let elId = el.getAttribute('id').split('test')[1];
        let siblingId = !sibling ? '' : sibling.getAttribute('id').split('test')[1];
        let dragType = el.parentNode.className.indexOf('tag-items') > -1 ? 'label' : 'group';
        // let prevGroups = JSON.parse(JSON.stringify(groups));
        if (dragType === 'label' && isManageModal) { return; }
        dispatch({
            'type': 'START_DROP',
            'data': {
                'dragType': dragType,
        ...
        // if (dragType !== 'label' && 
        //         this.isDefaultGroup(elId, siblingId, prevGroups)
        // ) {
        ...
        // }
        if (dragType === 'label') {
            dispatch(dragSortLabel({
                ids: [elId, siblingId ? siblingId : 0],
                groupId: this.dragSortLabelData()
            }));
        } else {
            dispatch(dragSortGroup(this.dragSortGroupData()));
        }
    ...
    dragSortLabelData = () => {
        ...
        groups.map((element, index) => {
            if (element.f_group_name === selectGroupName) {
                selectGroupId = element.f_group_id;
            }
        })
    ...
    dragSortGroupData = () => {
        ...
        groups.map((element, index) => {
            data.ids.push(element.f_group_id);
            // data.sort.push(index);
        })
        return data;    
    }
    //是否拖动目标的停靠位置是系统默认分组
    isDefaultGroup = (elId, siblingId, prevGroups) => {
        let replaceTarget;
        if (!siblingId) {
            replaceTarget = prevGroups[prevGroups.length - 1];
        } else {
            prevGroups.map((element, index) => {
                if (siblingId == element.f_group_id) {
                    replaceTarget = prevGroups[index - 1];   
                }
            })
        }

        if (replaceTarget.f_isdefault) {
            return true;
        } else {
            return false;
        }
    ...
    isNoGrag = (evt) => {
        ...
        let isHasDefautl;
        // isHasDefautl = groups[groups.length - 1].f_isdefault;
        // console.log(isHasDefautl + ':' + sibling.innerHTML);
        // if (el.getAttribute('data-default')) {
        // //拖拽目标是系统分组
        //     return false;
        // } else 
        if (evt.dragged.parentNode.className.indexOf('tag-items') > -1 && isManageModal) {
        //拖拽目标是批量管理模式的标签
            return false;
        } else {
            return true;
    ...
    setGroup = () => {
        const { dispatch } = this.props;
        const { manageModalLabels } = this.props.postsByReddit;

        if (!manageModalLabels.length) {
            Message.error('请先选择需要设置分组的标签!');
            return;
        }
        dispatch(swichVisible(true, 'isSetGroupVisible'));
        // if (!setGroupName) {
        //     dispatch({
        //         type: 'SET_GROUP_WARN',
        //         data: '请选择目标分组'
    ...
    sureSetGroup = () => {
        const {dispatch} = this.props;
        const { setGroupName } = this.props.postsByReddit;
        const { manageModalLabels } = this.props.postsByReddit;
        const { groups } = this.props.postsByReddit.groupList;
        let groupId;
        ...
        if (!setGroupName) {
            ...
            groups.map((element, index) => {
                if (element.f_group_name == setGroupName) {
                    groupId = element.f_group_id;
                }
            })
            let data = {
                ids: manageModalLabels,
                groupId: groupId
            };
            dispatch(multiSetGroup(data));
        }
    }
    hideNewSetGroup = () => {
        const {dispatch} = this.props;
    ...
    multiDel = () => {
        ...
        if (!manageModalLabels.length) {
            Message.error('请先选择需要删除的标签！');
        } else {
            dispatch(swichVisible(true, 'isConfirmMultiDelLabel'));
        }
    }
    sureMultiDelLabel = () => {
        const { dispatch } = this.props;
        const manageModalLabels = this.props.postsByReddit.manageModalLabels;
        dispatch(multiDelLabel({
            ids: manageModalLabels
        })) 
    }
    hideConfirmMultiDelLabel = () => {
        const { dispatch } = this.props;
    ...
    creatSetGroupFooter() {
        let setGroupWarn = this.props.postsByReddit.setGroupWarn;
        return [
            <div className="ec-modal-tip" style={{
                display: setGroupWarn ? 'block' : 'none'
            }}>{ setGroupWarn }</div>,
            <div className="ec-btn-group"> 
                <Button key="submit"
                        type="primary"
                        loading={this.state.loading}
                        onClick={this.sureSetGroup.bind(this)}>
                    { this.props.postsByReddit.isPosting ? `确定中...` : `确定` }
                </Button>
                <Button key="back"
                        type="ghost"
                        onClick={ this.hideNewSetGroup.bind(this, "visible") }>
                    取消
                </Button>
            </div>
        ]   
    ...
    sureDelLabels() {
       const {dispatch} = this.props;
       const {manageModalLabels} = this.props.postsByReddit;
       dispatch(swichVisible(false, 'isConfirmDelLabels'));
       if (!manageModalLabels.length) {
           Message.error('请先选择需要删除的标签！');
       }
    }
    hideConfirmDelLabels() {
        const {dispatch} = this.props;
        dispatch(swichVisible(false, 'isConfirmDelLabels'));
    }
    showCancelLabels() {
        const {dispatch} = this.props;
        dispatch(swichVisible(true, 'isConfirmDelLabels'));
    }
    render() {
        const {type} =  this.props.postsByReddit.corpData;
        ...
        let classStringColorUlLabel = 'tag-items container ' + ( selectGroup ? selectGroup.f_style : 'c1');
        ...
        //是否为系统分组
        let fIsdefault = selectGroup ? selectGroup.f_isdefault : 0;
            ...
            <div className={ 
                ...
                <Modal width="440px" maskClosable={ false }
                    ...
                    <div className="grouplabel-number" >
                        您还可以创建<span>{ remainGroupNum <= 0 ? 0 : remainGroupNum }</span>个分组
                    </div>
                </Modal>
                ...
                <Modal width="440px" maskClosable={ false }
                           title="删除标签"
                           visible={this.props.postsByReddit.isConfirmMultiDelLabel}
                           wrapClassName="vertical-center-modal"
                           onOk={this.sureMultiDelLabel.bind(this)}
                           onCancel={this.hideConfirmMultiDelLabel}>
                ...
                <Modal width="440px" maskClosable={ false }
                       title="设置分组"
                       visible={ isSetGroupVisible }
                       wrapClassName="vertical-center-modal"
                       footer={ this.creatSetGroupFooter() }
                       onCancel={ this.hideNewSetGroup.bind(this) }>
                   <Select 
                        showSearch
                        style={{ width: 400}}
                        placeholder="指定一个分组"
                        value={ setGroupName || undefined }
                        dropdownStyle={{height: 100}}
                        onChange={this.handleSetGroupSelect.bind(this)}>
                    {
                        groups && groups.filter((item, index) => {
                            return item.f_group_name !== selectGroupName;
                        }).map((item, index) => {
                            return (<Select.Option value={item.f_group_name} 
                                        key={'option' + index}>
                                    {item.f_group_name}
                                </Select.Option>);
                        })
                    ...
                    <div className="grouplabel-number" >
                       已选择<span>{ manageModalLabels.length }</span>个标签
                    </div>
                </Modal>
                <Modal width="440px" maskClosable={ false }
                           title="删除标签"
                           visible={this.props.postsByReddit.isConfirmDelLabels}
                           wrapClassName="vertical-center-modal"
                           onOk={this.sureDelLabels.bind(this)}
                           onCancel={this.hideConfirmDelLabels.bind(this)}>
                ...
                <div className="grouping-sorting-container">
                    <div className="grouping-sorting-box" style={{height: type == 0 ? 
                        ...
                        <div className="grouping-sorting-left">
                            ...
                            <ul className="grouping-sorting-name container" ref={this.sortDecorator}>
                                {
                        ...
                        <div className={labelList && labelList.length ? "grouping-sorting-right" : "grouping-sorting-right empty-tag" }>
                            ...
                            {
                                selectGroupName ?
                                    <div className="grouping-sorting-right-head">
                                    ...
                                    (selectGroupName ?
                                        (
                                            ...
                                            <div className="add-tag">
                                                ...
                                                <a href="javascript:;"
                                                   className="ec-link-text manage-modal"
                                                   onClick={this.switchManageModal.bind(this, false)}>退出管理</a>
                                            </div> : 
                                            <div className="add-tag">
                                                <Button type="ghost"
                                                        onClick={this.showNewLabelDialog.bind(this)}>
                                                ...
                                                <a href="javascript:;"
                                                   className="ec-link-text manage-modal"
                                                   onClick={this.switchManageModal.bind(this, true)}>批量管理</a>
                                            </div>

                                        ): null)
                            }
                            <ul className={classStringColorUlLabel} ref={this.sortDecorator}>
                                {
                            ...
                            </ul>
                            {
                                !groups || groups.length == 0 ?  
                                    ...
                                    (!labelList || labelList.length == 0 ?
                                        <div className={selectGroupName ? 
                                                "no-grouping center-no-grouping" : 
                                                "no-grouping"}>
                ...
                <Modal width="460px" 
                    ...
                    wrapClassName="vertical-center-modal" 
                    maskClosable={ false }
                    visible={this.props.postsByReddit.isColorVisible} 
                    footer={[
                        ...
                        <div key={ Math.random() + 2 } className="ec-btn-group">
                            ...
                            <Button key="back" type="ghost" onClick={this.handleCancelColorDig.bind(this, 'colorVisible')}>取消</Button></div>
                        ]} 
