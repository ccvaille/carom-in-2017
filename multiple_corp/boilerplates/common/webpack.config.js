...
const config = require('{{relativeDepth}}/config');
const baseConfig = require('{{relativeDepth}}/webpack.base.config');

const { __DEV__, __PROD__, __TEST__ } = config.globals;
const APP_ENTRY = path.resolve(__dirname, './client/index.js');

baseConfig.entry.{{entryName}} = __DEV__
...
if (__DEV__) {
    ...
    baseConfig.output.publicPath = 'https://1.staticec.com/{{basePath}}';
    baseConfig.plugins.push(
        new HtmlWebpackPlugin({
...
if (!__TEST__) {
    baseConfig.output.path = path.resolve('../../dist/{{basePath}}');
}
